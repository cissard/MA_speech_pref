...
DB$response_mode<-factor(DB$response_mode) #check that the experimental method doesn't make a difference
DB$test_lang<-factor(DB$test_lang) #check that the language doesn't make a difference (true speech preference and not pref for native language)
...
contrasts(DB$test_lang)<-contr.sum(2)
#dummy coding for method: each level is compared to the reference level (eye-tracking, 2nd group), the intercept corresponds to the cell mean of the reference group (eye-tracking).
contrasts(DB$response_mode)=contr.treatment(3, base=2)

#check that the method used doesn't make a difference (i.e. eye-tracking vs NIRS vs behavior, coded in the response_mode variable)
base_model <-rma.mv(g_calc, g_var_calc, mods = response_mode, random = ~ 1 | study_ID/same_infant, data=DB, weighted=TRUE, method = "ML", subset=!DB$outlier & !is.na(DB$response_mode))
...
#full model
full_model <-rma.mv(g_calc, g_var_calc, mods=~ naturalness, random = ~ 1 | study_ID/same_infant, data=DB, weighted=TRUE, method = "ML", subset=!DB$outlier)
...
boxplot(DB$g_calc~DB$test_lang)
tapply(DB$g_calc,DB$test_lang, mean,na.rm=T)
tapply(DB$g_calc,DB$test_lang, sd,na.rm=T)

```
```{r plots}
forest.rma(full_model, main = 'forest plot of effect sizes', xlab = 'Hedge\'s g', slab = DB$short_cite[!is.na(DB$g_calc)])

```
```{r publication bias}
fun.fig <- funnel(base_model, cex=1.5, xlab='Hedge\'s g', ylab="Standard Error of Effect Size g", digits=2, main="Funnel plot speech preference")
