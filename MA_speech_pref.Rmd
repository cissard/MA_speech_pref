...
DB$outlier[DB$d_calc > mean(DB$d_calc, na.rm = TRUE) + 3*sd(DB$d_calc, na.rm = TRUE) | DB$d_calc < mean(DB$d_calc, na.rm = TRUE) - 3*sd(DB$d_calc, na.rm = TRUE)]<-T #ac this code does exactly the same as the code below but faster
# for (i in 1:nrow(DB)){
#    db = DB[i,]
...
write.csv(DB,'speech_pref_full_DB.csv') #ac I recommend moving all of the above to another file, which generates the DB. And in this manuscript just read it in
...
#method
method <- boxplot(DB$g_calc~DB$method, main = 'ES as a function of method') #what is the difference between preferential listening and central fixation?
points(DB$g_calc~jitter(as.numeric(DB$method)))


#naturalness
ESnatural <- boxplot(DB$g_calc~DB$natural, main = 'ES as a function of naturalness')
points(DB$g_calc~jitter(as.numeric(DB$natural)))

#vocal
ESvocal <- boxplot(DB$g_calc~DB$vocal, main = 'ES as a function of vocalness')
points(DB$g_calc~jitter(as.numeric(DB$vocal)))
...
```{r publication bias}
#add symmetrize
tf <- trimfill(simple_model) #ac this model is fit in line 445; I think you can still knit it perhaps because you have the model in your global environment, but this document is not reproducible as is... You should fit the model before you do trimfill
...
base_model <- rma.mv(g_calc, g_var_calc, random = ~ 1 | study_ID/same_infant, data=DB, weighted=TRUE, method = "REML", subset=!DB$outlier) #ac weighted is not a functional parameter (ie it's just ignored in rma.mv) -- I suspect you don't need it because you are entering vi (which is not weights)

method_model <- rma.mv(g_calc, g_var_calc, mods = ~method, random = ~ 1 | study_ID/same_infant, data=DB, weighted=TRUE, method = "REML", subset=!DB$outlier & !is.na(DB$method)) #ac weighted is not a parameter
...
mycols
plot(DB$g_calc~DB$mean_age_1,col=mycols[DB$method],pch=20,cex=.7, legend=TRUE) #"legend" is not a graphical parameter
...
full_model <-rma.mv(g_calc, g_var_calc, mods= ~ natural*test_lang*agec, random = ~ 1 | study_ID/same_infant, data=DB, weighted=TRUE, method = "REML", subset=!DB$outlier, slab =DB$short_cite) #ac weighted is not a parameter
