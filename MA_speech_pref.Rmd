...
affiliation:
  - id            : "1"
    institution   : "Laboratoire de Sciences Cognitives et Psycholinguistique, École Normale Superieure, Département d'Études Cognitives"
  - id            : "2"
...
Single experiments tell us about what a specific group of participants, presented with a specific set of stimuli, at a specific point in time has done. Meta-analyses are the following step because they provide a principled statistical approach to integrate those individual and specific results into a larger picture. By aggregating the numerous individual studies of a literature, meta-analyses gain statistical power. As a result, meta-analyses can reveal small effects that are difficult to show in individual experiments. By integrating data across different laboratories, they provide evidence for the generalizability of effects, and facilitate comparisons between experimental results. A systematic review also allows us to identify empirical gaps (e.g., age groups that are under-represented but are crucial to tease apart two factors) and conceptual gaps (e.g., use of stimuli that systematically confounds two or more of those explanations). As a result, meta-analyses are a moment of self-reflection for the field [@cox_bayesian_2022;@nguyen_systematic_2022].
...
```{r prisma, include=T, out.width = "75%", fig.cap = "PRISMA flowchart summarizing the literature review and selection process."}
...
For **familiarity**, we considered the language in which the speech sounds were recorded (native or foreign). Because younger infants respond similarly to their native language and foreign languages within the same rhythmic class [@bertoncini_morae_1995; @mehler_precursor_1988; @nazzi_language_1998], infants would be expected to respond to a non-native language from the same rhythm class as their native native language similarly to a native language. We therefore took a careful look at our database to see, in the studies that used a foreign language as target speech sounds, whether the language belonged to the same rhythmic class as the infants’ native language or not. We found that all these studies but one used Japanese with English-learning infants, and the remaining one used Mandarin Chinese. Since Japanese is mora-timed, Mandarin syllable-timed, and English  stress-timed language, it turns out that all of the literature using a non-native language as target speech used samples from a different rhythmic class from the infants' native language. Given that there is no variability in rhythmicity, we did not code for linguistic rhythm. Also, according to standard interpretation, we can assume that even newborns would have distinguished the non-native speech from their own native language [@gasparini_quantifying_2021].

```{r TableStim, results="asis", include=T}

table_stim_vocal=rbind(
 cbind( "English-learning infants tested \\ with English and animal calls", "English-learning infants tested \\ with English and band-pass-filtered speech"),
 cbind( "English-learning infants tested \\ with Mandarin and animal calls", "English-learning infants tested \\ with Mandarin and band-pass-filtered speech")
)

row.names(table_stim_vocal)<-c("Native","Foreign")
colnames(table_stim_vocal)<-c("Natural, Vocal","Artificial, Vocal")

apa_table(table_stim_vocal,  align='cc',  caption = "Examples of contrasts classified into each target and competitor category.", escape= T)

table_stim_nonvocal=rbind(
 cbind( "English-learning infants tested \\ with English and heartbeat", "English-learning infants tested \\ with English and white noise"),
 cbind( "English-learning infants tested \\ with Mandarin and heartbeat", "English-learning infants tested \\ with Mandarin and white noise")
)

row.names(table_stim_nonvocal)<-c("Native","Foreign")
colnames(table_stim_nonvocal)<-c("Natural, Non-vocal","Artificial, Non-vocal")

apa_table(table_stim_nonvocal,  align='cc',  caption = "Examples of contrasts classified into each target and competitor category.", escape= T)

# grid.arrange(table_stim_vocal, table_stim_nonvocal, ncol=1, nrow=2)
```
...
```{r TableVocal, resultsw='asis', include=T} 
...
Our meta-analysis synthesizes the available literature on infants’ preference for speech sounds. When all experiments were considered together with no moderators, we found a sizable intercept (g=`r base_model$reg_table$b.r`, g=`r tf$beta[1,1]` when taking the publication bias into account), which was still significant after correcting for the publication bias. Our meta-analysis shows that this preferential processing of speech sounds is observable from birth on. It is important to stress that this conclusion is not trivial because others have said similar things in the past: One key advantage of meta-analysis over conclusions drawn from individual studies or unsystematic narrative reviews is that we can actually measure the likely presence of publication bias (which *is* present in these data, as we discuss below), and furthermore compensate for this bias statistically, to see if an effect remains significant after doing so [@cristia_meta-analytic_2022; see also @ioannidis_why_2005].
