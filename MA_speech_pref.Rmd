...
outliers<-filter(DB,outlier==T)
outliers

#Not considered as outliers but still high
highES<-filter(DB,g_calc>2)
highES
...
```{r plots}
forest.rma(full_model, main = 'forest plot of effect sizes', xlab = 'Hedge\'s g', slab = DB$short_cite[!is.na(DB$g_calc)&DB$outlier==F]) 
