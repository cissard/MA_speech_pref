...
DB$outlier <- F #create the variable, and sets as no by default (majority of cases hopefully!)
for (i in 1:nrow(DB)){
   db = DB[i,]
...
stripchart(DB$d_calc, main = 'Distribution of ES')
boxplot.stats(c(mean(DB$d_calc,na.rm=T),sd(DB$d_calc,na.rm=T)))
boxplot(DB$d_calc)
```
...
contrasts(DB$naturalness)=contr.sum(2) 
#contrast coding for species and social as well
contrasts(DB$species)<-contr.sum(2)
contrasts(DB$social)<-contr.sum(2)
...
```{r models}
#descriptives for results write-up
tapply(DB$g_calc,DB$mean_age_1,mean)#ES by age
...
#base model 
base_model <-rma.mv(g_calc, g_var_calc, mods=agec ,random = ~ 1 | study_ID/same_infant,data=DB, weighted=TRUE, method = "ML",subset=!DB$outlier)

plot(unique(DB$mean_age_1)[1:33],tapply(DB$g_calc,DB$mean_age_1,mean), main = 'ES as a function of age', xlab = 'age (days)', ylab = 'g_calc')

#subset selection
DB$periph<-ifelse(!is.na(DB$peripherality),T,F)

#full model
full_model <-rma.mv(g_calc, g_var_calc, mods=~ naturalness*species*agec + social, random = ~ 1 | study_ID/same_infant,data=DB, weighted=TRUE, method = "ML")
anova(base_model,full_model)
summary(full_model)


#Is there publication bias?
#funnel plot based on NRV-difference scores: for studies that report both directions: calculate an NRV score as "yes" minus "no" (and average SD of the SD-yes and SD-no) to see if there may indeed be selective reporting of asymmetries

#select studies that report both directions 
#(filter out study that are unidirectional manually)
DB.periph<-DB[DB$periph==T & DB$study_ID!="Liu2016" & DB$study_ID!="Sundara2011" & DB$study_ID!="Tsao2004" & DB$study_ID!="Polka1996",]

#match the two directions in one datafile by merge
DB.periph$mean_age_months_rd <-round(DB.periph$mean_age_months,0)#needed for match
DB.toperiph<-DB.periph[DB.periph$peripherality==1,]
DB.fromperiph<-DB.periph[DB.periph$peripherality==0,]
DB.periph<-merge(DB.toperiph[,c("study_ID","expt_num","native_lang","test_lang","contrast_sampa","g_calc","g_var_calc","mean_age_months_rd","exposure_phase")],DB.fromperiph[,c("study_ID","expt_num","native_lang","test_lang","contrast_sampa","g_calc","g_var_calc","mean_age_months_rd","exposure_phase")],by=c("study_ID","expt_num","native_lang","test_lang","mean_age_months_rd","exposure_phase"))

#calculate difference scores
DB.periph$ToMinFrom.g<-DB.periph$g_calc.x-DB.periph$g_calc.y
DB.periph$ToMinFrom.g_var<-sqrt((DB.periph$g_var_calc.x^2+DB.periph$g_var_calc.y^2)/2)

#can't do rma.mv for regtest, therefore rma.uni
periph.asym <- rma(ToMinFrom.g, ToMinFrom.g_var, mods=~ test_lang*mean_age_months_rd + exposure_phase,data=DB.periph, weighted=TRUE, method = "ML")

regtest(periph.asym)
fun.fig<-funnel(periph.asym,cex=1.5,ylab="Standard Error of Effect Size g",digits=2,main="Funnel plot speech preference")

#Figure
pdf("Fig1.pdf",height=5,width=7)
par(mfrow=c(1,2))
boxplot(DB$g_calc~DB$peripherality1,col="grey",xlab = "Peripherality",ylab="Hedge's g",
        main="Effect sizes by testing direction",cex=1.5)
funnel(periph.asym,cex=1.5,ylab="Standard Error of Effect Size g",digits=2,main="Funnel plot peripherality analysis")
dev.off()

#calculate regression weight of studies that awere conducted by supporters of NRV model
