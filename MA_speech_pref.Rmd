...
```{r forest, include=T, fig.cap = "Forest plot of effect sizes available in the literature, along with their respective moderator status. The average effect size is plotted on the bottom line."}
...
We found a total of `r length(papers)` publications (labeled with an asterisk in the reference list) reporting `r length(unique(DB2$same_infant))` experiments, for a total of `r sum(temp$n)` infants from `r round(min(DB2$mean_age_1)/30.44)` to `r round(max(DB2$mean_age_1)/30.44)` months (`r min(DB2$mean_age_1)` to `r max(DB2$mean_age_1)` days), and `r length(DB2$g_calc)` effect sizes, see Figure \@ref(fig:forest) (See Supplementary Information SI1 for a complete description of the database).
...
Second, the distribution of effect sizes in the literature is consistent with publication bias, in view of a strong asymmetry of the funnel plot. The trim-and-fill method suggested `r tf$k0` points may be missing. Given that we have `r length(DB2$g_calc)` effect sizes in total, a fifth more would be missing. The missing experiments are in the negative section, i.e., a preference *against* speech. The literature being biased toward positive effect sizes, the true effect size might be smaller than the average one we found with the available literature (see vertical line on Figure \@ref(fig:bias) for an estimate of the true effect size). To correct this bias, we invite researchers to use registered reports, in which manuscript are submitted before data are collected [@kiyonaga_practical_2019]. Our dataset can be community-augmented, and we invite researchers investigating this phenomenon to complement it with any data they would have (https://osf.io/4stz9/?view_only=d0696591ebf34bfc8430f848cd945ca8), whatever the results and publication status. 
...
knitr::include_graphics("/Users/Cecile/Documents/MA_speech_pref/figures_intro/PRISMA.png")
```
