...
for_plot = forest.rma(simple_model, annotate = FALSE, xlim = c(-4.5,3), alim = c(-3,3), top=0, xlab = 'Effect size (Hedges\' g)', mlab = "", steps = 7, fonts = 'mono', cex = 0.4, cex.axis = 0.6, cex.lab = 0.7) 
#dev.off()
```
...
fun.fig <- funnel(tf, cex=1.5, xlab='Hedges\' g', ylab="Standard Error of Effect Size g", digits=2, legend=TRUE)
...
Experimental method is known to be sizable in the cognitive developmental literature [@bergmann_promoting_2018]. We assessed that it did not impact our results by residualizing effect sizes from experimental method (Supplementary figure S2). 
...
```{r suplementary}
model_vocal <- robu(g_calc ~ vocal*mean_age_1, data=DB2, modelweights = "CORR", studynum = same_infant, var.eff.size = g_var_m, small = T)
...
model_lang <- robu(g_calc ~ test_lang*mean_age_1, data=DB2, modelweights = "CORR", studynum = same_infant, var.eff.size = g_var_m, small = T)

model_age <- robu(g_calc ~ mean_age_1, data=DB2, modelweights = "CORR", studynum = same_infant, var.eff.size = g_var_m, small = T)
