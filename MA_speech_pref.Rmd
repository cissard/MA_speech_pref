...
base_model <- rma.mv(g_calc, g_var_calc, random = ~ 1 | study_ID/same_infant, data=DB, method = "REML", subset=!DB$outlier) #ac weighted is not a functional parameter (ie it's just ignored in rma.mv) -- I suspect you don't need it because you are entering vi (which is not weights)
...
full_model <-rma.mv(g_calc, g_var_calc, mods= ~ natural*test_lang*agec, random = ~ 1 | study_ID/same_infant, data=DB, method = "REML", subset=!DB$outlier, slab =DB$short_cite)
