...
The median sample size at present is `r median(DB2$n)`, which is close to the field standard [@bergmann_promoting_2018] but much lower than current recommendations [@oakes_sample_2017]. Unjustified sample sizes can inadvertently lead to questionable research practices [@simmons_false-positive_2011], known to increase false positives. Our meta-analysis provides the average effect size across the literature, which will allow researchers to run power analyses to determine the sample size they need.
